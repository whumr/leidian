...
void SneakyJoystick::updateVelocity(Point point)
{
	...
	if(isDPad){
		...
		angle = round_angle(angle / anglePerSector) * anglePerSector;
	}
...
bool SneakyJoystick::TouchBegan(Touch *touch, Event *event)
{
...
void SneakyJoystick::TouchMoved(Touch *touch, Event *event)
{
	Point location = Director::getInstance()->convertToGL(touch->getLocationInView());
	location = this->convertToNodeSpace(location);
	this->updateVelocity(location);
...
void SneakyJoystick::TouchEnded(Touch *touch, Event *event)
{
...
void SneakyJoystick::TouchCancelled(Touch *touch, Event *event)
{
